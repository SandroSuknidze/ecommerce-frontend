...
function Index() {
    ...
    useEffect(() => {
        axiosInstance.get('/cookie-policy')
            .then((response) => {
                setCookieText(response.data.text)
            })
            .catch((error) => {
                console.log(error)
            })
        ...
        <>
            <div className="max-w-[1260px] px-[30px] md:px-[15px] m-auto">
                <div className="mx-auto flex flex-col justify-center py-[60px] text-center">
                    <h1 className="leading-[47px] mb-[5px] text-[45px]">{t('cookiePolicy')}</h1>
                    <nav>
                        <ol className="text-[14px] text-55black mt-[10px]">
                            <li className="inline">
                                <Link href="/">{t('home')} / </Link>
                            </li>
                            <li className="inline">{t('cookiePolicy')}</li>
                        </ol>
                    </nav>
                </div>
                {cookieText ? (
                    <div dangerouslySetInnerHTML={{ __html: cookieText }} className=" m-auto custom-class" />
                ) : (
                    <div>

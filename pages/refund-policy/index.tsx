...
function Index() {
    ...
    const [refundText, setPrivacyText] = useState('')

    useEffect(() => {
        axiosInstance.get('/refund-policy')
            .then((response) => {
        ...
        <>
            <div className="max-w-[1260px] px-[30px] md:px-[15px] m-auto">
                <div className="mx-auto flex flex-col justify-center py-[60px] text-center">
                    <h1 className="leading-[47px] mb-[5px] text-[45px]">{t('refundPolicy')}</h1>
                    <nav>
                        <ol className="text-[14px] text-55black mt-[10px]">
                            <li className="inline">
                                <Link href="/">{t('home')} / </Link>
                            </li>
                            <li className="inline">{t('refundPolicy')}</li>
                        </ol>
                    </nav>
                </div>
                {refundText ? (
                    <div dangerouslySetInnerHTML={{ __html: refundText }} className=" m-auto custom-class" />
                ) : (
                    <div>
